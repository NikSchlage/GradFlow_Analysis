...
res_ratio <- read.table("../AIC_fit_results_cf_ratio_6_aaqb_id.txt")
...
tikzDevice::tikz( file = "AIC_fit_results_cf_ratio_6_aaqb_id_Nconfs563_Nsamples1_msbar.tex", standAlone = TRUE, width = 4.5, height = 3,
...
plotwitherror( x=my_gft[-1], y=ratio_msbar, dy=ratio_err1_msbar, mdy=ratio_err2_msbar,
               ...
               cex=1.2,
               xlab="$t_\\mathrm{gf}$",
               ylab="$(Z_\\chi / \\xi_\\chi) \\cdot \\langle O_{aa}^{qb}\\, O_\\mathbb{1}\\rangle (t_\\mathrm{gf}) / \\langle O_\\mathbb{1} \\, O_\\mathbb{1}\\rangle (t_\\mathrm{gf})$", log="xy" )

title(paste0("$\\mathrm{cA211a.30.32,}$ $N_\\mathrm{samples}=1,$ $N_\\mathrm{confs}=", Nconfs, "$"), line=0.6)

dev.off()
# Compile the tex file
tools::texi2dvi( "AIC_fit_results_cf_ratio_6_aaqb_id_Nconfs563_Nsamples1_msbar.tex", pdf=TRUE )