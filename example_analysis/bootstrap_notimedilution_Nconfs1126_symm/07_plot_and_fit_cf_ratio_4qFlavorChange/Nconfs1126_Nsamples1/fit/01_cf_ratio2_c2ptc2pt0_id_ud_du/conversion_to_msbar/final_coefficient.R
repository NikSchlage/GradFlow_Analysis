...
for( i in 2:length(my_gft) ){
  ...
  ratio_msbar      <- c( ratio_msbar, res_ratio[i,2] * (Zchi_MeanVal[[1]][i] / xichi[[1]])^2 )
  ratio_err1_msbar <- c( ratio_err1_msbar, res_ratio[i,4] * (Zchi_MeanVal[[1]][i] / xichi[[1]])^2 )
  ratio_err2_msbar <- c( ratio_err2_msbar, res_ratio[i,3] * (Zchi_MeanVal[[1]][i] / xichi[[1]])^2 )
...
tikzDevice::tikz( file = "AIC_fit_results_cf_ratio2_c2ptc2pt0_id_Nconfs1126_Nsamples1_msbar.tex", standAlone = TRUE, width = 4.5, height = 3,
...
plotwitherror( x=my_gft[-1], y=ratio_msbar, dy=ratio_err1_msbar, mdy=ratio_err2_msbar,
               ...
               ylab="$(\\mathring{Z}_\\chi / \\xi_\\chi)^2 \\cdot \\langle O_\\mathbb{1}\\, O_\\mathbb{1}\\rangle (t_\\mathrm{gf}) / \\langle O_\\mathbb{1} \\, O_\\mathbb{1}\\rangle (0)$", log="xy" )

title(paste0("$\\mathrm{cA211a.30.32,}$ $N_\\mathrm{samples}=1,$ $N_\\mathrm{confs}=", Nconfs, "$"), line=0.6)

dev.off()
# Compile the tex file
tools::texi2dvi( "AIC_fit_results_cf_ratio2_c2ptc2pt0_id_Nconfs1126_Nsamples1_msbar.tex", pdf=TRUE )