...
res_ratio <- read.table("../AIC_fit_results_cf_ratio_5_vvcc_id_ud_du_no2ptflow.txt")
...
for( i in 2:length(my_gft) ){
  ...
  ratio_msbar      <- c( ratio_msbar,      res_ratio[i,2] * ( ((1/(4*pi*my_gft[i])^2) * Zchi_MeanVal[[1]][i]) / xichi[[1]])^2 )
  ratio_err1_msbar <- c( ratio_err1_msbar, res_ratio[i,4] * ( ((1/(4*pi*my_gft[i])^2) * Zchi_MeanVal[[1]][i]) / xichi[[1]])^2 )
  ratio_err2_msbar <- c( ratio_err2_msbar, res_ratio[i,3] * ( ((1/(4*pi*my_gft[i])^2) * Zchi_MeanVal[[1]][i]) / xichi[[1]])^2 )
...
tikzDevice::tikz( file = "AIC_fit_results_cf_ratio_5_vvcc_id_ud_du_Nconfs1126_Nsamples1_msbar_no2ptflow.tex", standAlone = TRUE, width = 4.5, height = 3,
...
plotwitherror( x=my_gft[-1], y=ratio_msbar, dy=ratio_err1_msbar, mdy=ratio_err2_msbar,
               ...
               ylab="$(\\mathring{Z}_\\chi / \\xi_\\chi)^2 \\cdot \\langle O_{vv}^{cc}\\, O_\\mathbb{1}\\rangle (t_\\mathrm{gf}) / \\langle O_\\mathbb{1} \\, O_\\mathbb{1}\\rangle (0)$", log="xy" )

title(paste0("$\\mathrm{cA211a.30.32,}$ $N_\\mathrm{samples}=1,$ $N_\\mathrm{confs}=", Nconfs, "$"), line=0.6)

dev.off()
# Compile the tex file
tools::texi2dvi( "AIC_fit_results_cf_ratio_5_vvcc_id_ud_du_Nconfs1126_Nsamples1_msbar_no2ptflow.tex", pdf=TRUE )