...
for ( i in 1:21 ) {
  for ( j in 0:(x_u - 1) ) {
    ...
    fit_res_tmp <- fit.const( y = ratio_4_id_ud_du[[i]]$cf.tsboot$t0,
                              bsamples = ratio_4_id_ud_du[[i]]$cf.tsboot$t,
                              x = seq( j, length(ratio_4_id_ud_du[[i]]$cf.tsboot$t0 - 1), 1 ),
                              x.lower = j,
                              x.upper = x_u,
                              start.par = c(1),
                              useCov = TRUE,
...
for ( i in 1:21 ) {
  ...
  res_ratio_4_id_ud_du[[i]] <- cbind( ratio_val, ratio_err, redchisqr, pval )

  write.table(x=res_ratio_4_id_ud_du[[i]], file=paste0("fitres_tu", x_u , "_ratio_4_id_ud_du_tgf", my_gft[i] , ".txt"), append = FALSE, sep = " ", dec = ".", row.names = FALSE, col.names = TRUE)
}
...
tikzDevice::tikz(file = paste0("fitres_tu", x_u, "_ratio_4_id_ud_du_redchisqr.tex"), standAlone = TRUE, width = 5, height = 2)
par(mar = c(4.4, 5, 1.5, 0.5))

hadron::plotwitherror( x=seq(0-0.4, (x_u - 1)-0.4,1),
                       y=res_ratio_4_id_ud_du[[1]][,3],
                       xlim=c(-0.5,x_u - 0.5),
                       ylim=c(0,2),
                       xlab="$t_l$",
...
tools::texi2dvi(paste0("fitres_tu", x_u, "_ratio_4_id_ud_du_redchisqr.tex"), pdf=TRUE)

#---------------------------------------------------------------------------#

tikzDevice::tikz(file = paste0("fitres_tu", x_u, "_ratio_4_id_ud_du_pval.tex"), standAlone = TRUE, width = 5, height = 2)
par(mar = c(4.4, 5, 1.5, 0.5))

hadron::plotwitherror( x=seq(0-0.4, (x_u - 1)-0.4,1),
                       y=res_ratio_4_id_ud_du[[1]][,4],
                       xlim=c(-0.5,x_u - 0.5),
                       ylim=c(0,1),
                       xlab="$t_l$",
                       ylab="$p\\,\\,\\mathrm{value}$",
...
dev.off()
# Compile the tex file
tools::texi2dvi(paste0("fitres_tu", x_u, "_ratio_4_id_ud_du_pval.tex"), pdf=TRUE)

#---------------------------------------------------------------------------#

tikzDevice::tikz(file = paste0("fitres_tu", x_u, "_ratio_4_id_ud_du.tex"), standAlone = TRUE, width = 5, height = 2,
                  packages=c(getOption( "tikzLatexPackages" ),"\\usepackage{bbold}") )
par(mar = c(4.4, 5, 1.5, 0.5))

hadron::plotwitherror( x=seq(0,x_u - 1,1),
                       y=res_ratio_4_id_ud_du[[1]][,1],
                       dy=res_ratio_4_id_ud_du[[1]][,2],
                       xlim=c(-0.5,x_u - 0.5),
                       ylim=c(0.002,0.01),
                       xlab="$t_l$",
...
for ( i in 2:21 ) {
hadron::plotwitherror( x=seq(0,x_u - 1,1),
                       y=res_ratio_4_id_ud_du[[i]][,1],
                       dy=res_ratio_4_id_ud_du[[i]][,2],
                       rep=TRUE,
...
dev.off()
# Compile the tex file
tools::texi2dvi(paste0("fitres_tu", x_u, "_ratio_4_id_ud_du.tex"), pdf=TRUE)
