...
for ( i in 1:length(my_gft) ) {
  ...
  AIC_fit( Rdata=ratio_4_id_ud_du[[i]],
           gradflow_t=my_gft[i],
           filename=paste0("fit_cf_ratio4_vvqb_id_ud_du_gft", my_gft[i], "_Nsamples1"),
           #x_lab="$\\langle O_{vv}^{qb}\\, O_\\mathbb{1}\\rangle_{ud+du} (t_\\mathrm{gf}) / \\langle O_\\mathbb{1} \\, O_\\mathbb{1}\\rangle_{ud+du} (t_\\mathrm{gf})$" )
           x_lab="$\\frac{1}{2}\\langle O_{vv}^{qb}\\, O_{1}\\rangle_{ud+du} (t_\\mathrm{gf}) / \\langle O_{1} \\, O_{1}\\rangle_{ud+du} (t_\\mathrm{gf})$",
           t_i=0, t_f=5, tl_1=1, tl_2=4, tu_1=0, tu_2=5 )
}
