...
for ( i in 1:length(my_gft) ) {
  ...
  AIC_fit( Rdata=ratio_5_id_ud_du[[i]],
           gradflow_t=my_gft[i],
           filename=paste0("fit_cf_ratio5_vvcc_id_ud_du_gft", my_gft[i], "_Nsamples1"),
           #x_lab="$\\langle O_{vv}^{cc}\\, O_\\mathbb{1}\\rangle_{ud+du} (t_\\mathrm{gf}) / \\langle O_\\mathbb{1} \\, O_\\mathbb{1}\\rangle_{ud+du} (t_\\mathrm{gf})$" )
           x_lab="$\\frac{1}{2}\\langle O_{vv}^{cc}\\, O_{1}\\rangle_{ud+du} (t_\\mathrm{gf}) / \\langle O_{1} \\, O_{1}\\rangle_{ud+du} (t_\\mathrm{gf})$",
