...
my_Zchi_path <- "/home/nikolas/Documents/NPV/njjn_analysis/GRADIENT_FLOW/mixing_probe_src_oet_GRADFLOW/GRADFLOW_ANALYSIS/05_bootstrap_notimedilution_Nconfs1126_symm/01_boot_cf_data/Kchi/01_boot_kchi_data_Nconfs350/bootstrap_Zchi_Nconfs350_Nsamples1to5.RData"
load(my_Zchi_path)

res_ratio <- read.table("../AIC_fit_results_cf_ratio_4_vvqb_id_ud_du_no2ptflow.txt")
...
for( i in 2:length(my_gft) ){
  ...
  ratio_msbar      <- c( ratio_msbar,      res_ratio[i,2] * ( (my_kinop_treelevel_path[i,2] * Zchi_MeanVal[[1]][i]) / xichi[[1]] )^2 )
  ratio_err1_msbar <- c( ratio_err1_msbar, res_ratio[i,4] * ( (my_kinop_treelevel_path[i,2] * Zchi_MeanVal[[1]][i]) / xichi[[1]] )^2 )
  ratio_err2_msbar <- c( ratio_err2_msbar, res_ratio[i,3] * ( (my_kinop_treelevel_path[i,2] * Zchi_MeanVal[[1]][i]) / xichi[[1]] )^2 )
...
tikzDevice::tikz( file = "AIC_fit_results_cf_ratio_4_vvqb_id_Nconfs1126_Nsamples1_msbar_no2ptflow_treelevel.tex", standAlone = TRUE, width = 4.5, height = 3,
...
plotwitherror( x=my_gft[-1], y=ratio_msbar, dy=ratio_err1_msbar, mdy=ratio_err2_msbar,
               ...
               ylab="$(\\mathring{Z}_\\chi / \\xi_\\chi)^2 \\cdot \\langle O_{vv}^{qb}\\, O_\\mathbb{1}\\rangle (t_\\mathrm{gf}) / \\langle O_\\mathbb{1} \\, O_\\mathbb{1}\\rangle (0)$" )

title(paste0("$\\mathrm{cA211a.30.32,}$ $N_\\mathrm{samples}=1,$ $N_\\mathrm{confs}=", Nconfs, "$"), line=0.6)

dev.off()
# Compile the tex file
tools::texi2dvi( "AIC_fit_results_cf_ratio_4_vvqb_id_Nconfs1126_Nsamples1_msbar_no2ptflow_treelevel.tex", pdf=TRUE )